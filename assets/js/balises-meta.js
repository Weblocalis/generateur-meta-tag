...
function replace_submit() {
    ...
    // Remplace le contenu de l'élément avec l'ID "change_submit" par le nouveau bouton et la zone de texte
    document.getElementById("og_change_submit").innerHTML = '<input type="button" id="launch" value="Générer les balises Meta" onclick="gene_balise(this.form)" /><br><label for="balises_meta">Collez ce code dans la balise <code>&lt;head&gt;-&lt;/head&gt;</code> de votre page web :</label><br><textarea id="balises_meta" name="balises_meta" style="white-space: pre-wrap;" rows="12" cols="60" readonly></textarea>';
}
...
function gene_balise(form) {
    ...
    if (form.og_title.value.trim() !== "") {
        form.balises_meta.value += '<meta property="og:title" content="' + form.og_title.value + '">\n';
    }
    if (form.og_type.value.trim() !== "") {
        form.balises_meta.value += '<meta property="og:type" content="' + form.og_type.value + '">\n';
    }
    if (form.og_url.value.trim() !== "") {
        form.balises_meta.value += '<meta property="og:url" content="' + form.og_url.value + '">\n';
    }
    if (form.og_image.value.trim() !== "") {
        form.balises_meta.value += '<meta property="og:image" content="' + form.og_image.value + '">\n';
    }
